...
def radar_process_command(dsp, tracker, command, sim_data):
    ...
    if command == "scan":
        ...
        # Params: Ny, Nz, dy, dz, freq, steerAz, steerEl, steeringType, windowType, bw, tau
        af_db = dsp.calculate3DSpatialPattern(Ny, Nz, 0.05, 0.05, freq, steerAz, steerEl, steerType, taper, bw, tau)
        payload["rd_matrix"] = af_db.tolist()
        
        # 2. STAP Clutter Cancellation with Electronic Warfare (Jammers & Rain Attenuation)
        N = Ny 
        M = 16 
        v = 50.0 
        d = 0.05 
